...
# outliers<-abs(counts.normalized[,plasmid]-plasmid.mean)>3*plasmid.sd
# outliers<-counts$Gene %in% counts$Gene[outliers]
# counts<-counts[!outliers,]
# counts.normalized<-counts.normalized[!outliers,]
...
zm<-zm[order(rowSums(zm),decreasing=TRUE),]
zm<-zm[1:min(25,nrow(zm)),]
pal<-colorRampPalette(rev(brewer.pal(n = 11, name = "RdYlBu")[-4]))
newnames <- lapply(
  rownames(zm),
  function(x) bquote(italic(.(x))))
pheatmap(zm,col=pal(9),scale="none",cluster_cols = FALSE,cluster_rows = FALSE,cex=.75,breaks=seq(-4,4,length.out=9),cellwidth=8,cellheight=8,labels_row = as.expression(newnames))
dev.off()